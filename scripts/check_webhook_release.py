...
_REQUIRED_SDIST_DOCUMENTS = (
    ...
    "docs/adrs/README.md",
    "docs/adrs/ADR-0001-explicit-webhook-serializers-and-durable-envelopes.md",
    "docs/adrs/ADR-0002-versioned-webhook-signing-keys.md",
    "docs/adrs/ADR-0003-fail-closed-webhook-egress.md",
    "docs/adrs/ADR-0004-bounded-webhook-retry-and-redrive.md",
    "docs/adrs/ADR-0005-opt-in-webhook-runtime-and-administration.md",
)
...
def _webhook_source_files() -> tuple[str, ...]:
    source = _ROOT / "src" / "phoenix_os" / "webhooks"
    files = tuple(f"phoenix_os/webhooks/{path.name}" for path in sorted(source.glob("*.py")))
    if not files:
        raise RuntimeError("webhook package contains no Python modules")
    return files
    ...
    *,
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        required = {
            ...
            "phoenix_os/py.typed",
            *_webhook_source_files(),
        }
        missing = sorted(required - available)
        if missing:
            raise RuntimeError(
                f"wheel {wheel.name} is missing required package files: " + ", ".join(missing)
            )
...
def _validate_sdist(sdist: Path) -> None:
    ...
    required = {
        *_REQUIRED_SDIST_DOCUMENTS,
        "src/phoenix_os/__init__.py",
        "src/phoenix_os/py.typed",
        *{f"src/{name}" for name in _webhook_source_files()},
    }
    missing = sorted(required - relative)
    if missing:
        raise RuntimeError(f"sdist {sdist.name} is missing required files: " + ", ".join(missing))
...
endpoint = WebhookEndpoint("https://hooks.example.com/release")
policy = WebhookEgressPolicy("release.webhooks")
...
assert verify_webhook_signature(
    ...
    delivery_id="00000000-0000-4000-8000-000000000024",
    attempt="1",
    body=b"{{}}",
) is False
"""
...
def main() -> int:
    ...
    print("Running RFC-0024 regression and security suites.", flush=True)
    _run(
        (
            sys.executable,
            "-m",
    ...
    with tempfile.TemporaryDirectory(
        prefix="phoenix-webhook-release-",
    ) as temporary:
        ...
        print("Installing artifacts into isolated offline environments.", flush=True)
        _smoke_install(
            wheel,
            version=version,
            workspace=workspace,
    ...
    print("RFC-0024 webhook release gate passed.", flush=True)
    return 0
