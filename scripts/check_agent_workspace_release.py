...
_REQUIRED_WORKSPACE_TESTS = frozenset(
    {
        "tests/test_agent_workspace_administration.py",
        "tests/test_agent_workspace_adrs.py",
        "tests/test_agent_workspace_authorization.py",
        "tests/test_agent_workspace_backing.py",
        "tests/test_agent_workspace_cleanup.py",
        "tests/test_agent_workspace_cleanup_runtime.py",
        "tests/test_agent_workspace_context.py",
        "tests/test_agent_workspace_context_integration.py",
        "tests/test_agent_workspace_contracts.py",
        "tests/test_agent_workspace_migration_guidance.py",
        "tests/test_agent_workspace_observer.py",
        "tests/test_agent_workspace_recovery.py",
        "tests/test_agent_workspace_release_gate.py",
        "tests/test_agent_workspace_runtime_assembler.py",
        "tests/test_agent_workspace_security_review.py",
        "tests/test_agent_workspace_service.py",
        "tests/test_agent_workspace_store.py",
        "tests/test_agent_workspace_transfer.py",
        "tests/test_agent_workspace_transfer_runtime.py",
    }
)

_REQUIRED_WORKSPACE_MODULES = frozenset(
    {
        "phoenix_os/agent/workspace_administration.py",
        "phoenix_os/agent/workspace_authorization.py",
        "phoenix_os/agent/workspace_backing.py",
        "phoenix_os/agent/workspace_cleanup_runtime.py",
        "phoenix_os/agent/workspace_composition.py",
        "phoenix_os/agent/workspace_context.py",
        "phoenix_os/agent/workspace_contracts.py",
        "phoenix_os/agent/workspace_observer.py",
        "phoenix_os/agent/workspace_runtime.py",
        "phoenix_os/agent/workspace_service.py",
        "phoenix_os/agent/workspace_store.py",
        "phoenix_os/agent/workspace_transfer.py",
        "phoenix_os/agent/workspace_transfer_runtime.py",
    }
)
...
_REQUIRED_SDIST_DOCUMENTS = (
    ...
    "docs/adrs/ADR-0057-phoenix-owned-logical-paths-and-host-confinement.md",
    "docs/adrs/ADR-0058-authoritative-workspace-store-and-backing-boundary.md",
...
def _workspace_test_files() -> tuple[str, ...]:
    discovered = tuple(
        path.relative_to(_ROOT).as_posix()
        for path in sorted((_ROOT / "tests").glob("test_agent_workspace*.py"))
    )
    missing = sorted(_REQUIRED_WORKSPACE_TESTS - frozenset(discovered))
    if missing:
        raise RuntimeError(
            "agent-workspace regression suite is missing required tests: " + ", ".join(missing)
        )

    for relative in _COMPANION_TESTS:
        if not (_ROOT / relative).is_file():
            raise RuntimeError(f"required agent-workspace release test is missing: {relative}")
    return (*discovered, *_COMPANION_TESTS)


def _workspace_source_files() -> tuple[str, ...]:
    source = _ROOT / "src" / "phoenix_os" / "agent"
    files = tuple(f"phoenix_os/agent/{path.name}" for path in sorted(source.glob("workspace_*.py")))
    missing = sorted(_REQUIRED_WORKSPACE_MODULES - frozenset(files))
    if missing:
        raise RuntimeError(
            "agent-workspace package is missing required workspace modules: " + ", ".join(missing)
        )
    return files
...
def _validate_wheel(
    ...
    *,
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        if missing:
            raise RuntimeError(
                f"wheel {wheel.name} is missing required agent-workspace files: "
                + ", ".join(missing)
            )
...
def _validate_sdist(sdist: Path) -> None:
    ...
    if missing:
        raise RuntimeError(
            f"sdist {sdist.name} is missing required agent-workspace files: " + ", ".join(missing)
        )
...
async def main() -> None:
    ...
    namespace = WorkspaceNamespace("release")
    agent_id = AgentId("assistant")
    scope = agent_workspace_scope(namespace=namespace, agent_id=agent_id)
    artifact_id = ArtifactId(
        UUID("31000000-0000-0000-0000-000000000031")
    )

    assert (
        workspace_scope_resource(scope)
        == "agent-workspace:release/scope:agent:assistant"
    )
    assert workspace_artifact_resource(scope, artifact_id) == (
        "agent-workspace:release/scope:agent:assistant/"
        "artifact:31000000-0000-0000-0000-000000000031"
    )

    try:
        ArtifactLogicalPath("../escape")
    except ValueError:
        pass
    else:
        raise AssertionError("logical traversal must fail closed")

    now = datetime.now(UTC)
    content = b"packaged workspace release smoke"
    ...
    record = await store.write(
        ArtifactWriteRequest(
            ...
            logical_path=ArtifactLogicalPath("release/smoke.txt"),
            content=content,
    ...
    updated_content = b"packaged workspace release smoke updated"
    updated_digest = artifact_content_digest(updated_content)
    ...
    assert updated.content_digest == updated_digest
...
def main() -> int:
    ...
    print(
        "Running RFC-0031 agent-workspace contracts, authorization, backing, "
        "store, transfer, context, recovery, lifecycle, administration, "
        "migration, ADR, security-review, and release suites.",
        flush=True,
    )
    _run((sys.executable, "-m", "pytest", "-q", *_workspace_test_files()))

    with tempfile.TemporaryDirectory(prefix="phoenix-agent-workspace-release-") as temporary:
        ...
        print(
            "Installing both wheel forms offline and running isolated "
            "agent-workspace smoke validation.",
            flush=True,
        )
        _smoke_install(
            wheel,
    ...
    print(
        f"RFC-0031 agent-workspace release gate passed for {project_name} {version}.",
        flush=True,
    )
    return 0
